function createClient() {
    return new neurolib.ApiClient('tst-public-key', 'dIuCAwTeXn87m24HYE6uAIwSVzgEkTrQflrYlDBeIoeTIrhG1FVqj1v0h6u3nNSu');
}
...
QUnit.test("AddBookmark test", function( assert ) {
    testInteraction(assert, new neurolib.AddBookmark('user-1', 'item-1'));
});

QUnit.test("AddCartAddition test", function( assert ) {
    testInteraction(assert, new neurolib.AddCartAddition('user-1', 'item-1'));
});

QUnit.test("AddDetailView test", function( assert ) {
    testInteraction(assert, new neurolib.AddDetailView('user-1', 'item-1'));
});

QUnit.test("AddPurchase test", function( assert ) {
    testInteraction(assert, new neurolib.AddPurchase('user-1', 'item-1'));
});

QUnit.test("AddRating test", function( assert ) {
    testInteraction(assert, new neurolib.AddRating('user-1', 'item-1', -1));
});

QUnit.test("SetViewPortion test", function( assert ) {
    testInteractionPromise(assert, new neurolib.SetViewPortion(Math.random().toString(36).substring(7), 'item-1', 0.1));
    testInteractionCallback(assert, new neurolib.SetViewPortion(Math.random().toString(36).substring(7), 'item-1', 0.1));
});
...
function addInteractions(client) {
  ...
  for (i = 0; i < 50; i++) {
      for (j = 0; j < 50; j++)
        client.send(new neurolib.AddPurchase('user-' + i, 'item-' + j));
  }
}
...
QUnit.test("RecommendItemsToUser test", function( assert ) {
    testRecommendations(assert, new neurolib.RecommendItemsToUser('user-1', 5));
});

QUnit.test("RecommendItemsToItem test", function( assert ) {
    testRecommendations(assert, new neurolib.RecommendItemsToItem('item-1', 'user-1', 5));
});

QUnit.test("SearchItems test", function( assert ) {
    testRecommendations(assert, new neurolib.SearchItems('user-1', 'computer', 5));
});
...
QUnit.test("RecommendNextItems test", function( assert ) {
  ...
  var done = assert.async();
    var client = createClient();
    client.send(new neurolib.RecommendItemsToUser('user-1', 5, {'returnProperties': true}))
    .then(function(res) {
      testRecommendations(assert, new neurolib.RecommendNextItems(res.recommId, 5));
      done();
    });
});
